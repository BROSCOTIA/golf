      ...
      */}
      <div className="p-4 sm:p-5 space-y-3">
        ...
        <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between">
          ...
          <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-black text-base border shadow-md ${
            customer.sumOfStoreCreditBalance > 500
              ? 'bg-rose-950/90 text-rose-300 border-rose-600 ring-1 ring-rose-500/40'
              : 'bg-emerald-950/90 text-emerald-300 border-emerald-800'
          }`}>
            <DollarSign className={`w-4 h-4 shrink-0 ${customer.sumOfStoreCreditBalance > 500 ? 'text-rose-400' : 'text-emerald-400'}`} />
            <span>${customer.sumOfStoreCreditBalance.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
            {customer.sumOfStoreCreditBalance > 500 && (
              <span className="text-[9px] uppercase tracking-wider font-extrabold px-1.5 py-0.5 rounded bg-rose-600 text-white ml-0.5">
                Over $500
              </span>
            )}
          </div>
