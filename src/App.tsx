...
export default function App() {
  ...
  // Global Keyboard Shortcuts (Esc to close modals, / to search, Ctrl/Cmd+Enter to submit form)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // 1. 'Esc' to close active modals
      if (e.key === 'Escape') {
        ...
        if (isEmailFormModalOpen) {
          setIsEmailFormModalOpen(false);
          return;
        }
        if (isLiveSocketModalOpen) {
          setIsLiveSocketModalOpen(false);
          return;
        }
        if (isAutomatedAlertsModalOpen) {
          setIsAutomatedAlertsModalOpen(false);
          return;
        }
      ...
      // 2. '/' to focus search bar (if not already focused in an input/textarea/select)
      ...
      // 3. 'Ctrl+Enter' or 'Cmd+Enter' to submit Customer Add/Edit form
      if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
        if (isAddEditModalOpen && addEditFormRef.current) {
          e.preventDefault();
          if (typeof addEditFormRef.current.requestSubmit === 'function') {
            addEditFormRef.current.requestSubmit();
          } else {
            addEditFormRef.current.dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }));
          }
        }
      }
    };
  ...
  // Global Backup JSON Handler
  const handleGlobalBackup = () => {
    const backupData = {
      backupMetadata: {
        system: "Golf Town Store Credit Portal",
      ...
      emailTemplates: [
        {
          id: "gt-credit-notice-v1",
          name: "Official Golf Town Store Credit Deposit Notice",
          type: "Customer Notice",
          subject: "Golf Town Store Credit Deposit Notice - Action Required",
          senderName: "Golf Town Customer Support",
          senderEmail: "store-support@golftown.com",
          placeholders: ["{{RECIPIENT_NAME}}", "{{CUSTOMER_ID}}", "{{STORE_ID}}", "{{AMOUNT}}", "{{SECURE_DEPOSIT_URL}}"],
          content: "Dear {{RECIPIENT_NAME}},\n\nYou have an available store credit balance of {{AMOUNT}} on file for Store #{{STORE_ID}}.\n\nPlease follow your personalized secure link to review and authorize your credit deposit:\n{{SECURE_DEPOSIT_URL}}\n\nThank you for choosing Golf Town Canada."
        },
        {
          id: "gt-alberta-refund-receipt-v1",
          name: "Custom Receipt Refund Template (Alberta 5% GST)",
          type: "Refund Receipt",
          subject: "Golf Town Refund Receipt #{{RECEIPT_NO}} - Store #{{STORE_ID}}",
          senderName: "Golf Town Store #504 Calgary",
          senderEmail: "receipts@golftown.com",
          placeholders: ["{{CUSTOMER_NAME}}", "{{RECEIPT_NO}}", "{{STORE_ID}}", "{{SUBTOTAL}}", "{{GST_AMOUNT}}", "{{TOTAL_REFUND}}"],
          content: "GOLF TOWN CANADA - CUSTOM REFUND RECEIPT\nReceipt #: {{RECEIPT_NO}}\nCustomer: {{CUSTOMER_NAME}}\nStore ID: {{STORE_ID}}\nSubtotal: {{SUBTOTAL}}\nAlberta GST (5%): {{GST_AMOUNT}}\nTotal Refund Balance: {{TOTAL_REFUND}}"
        },
        {
          id: "gt-high-value-alert-v1",
          name: "Automated High-Value Credit Escalation Notice",
          type: "System Escalation",
          subject: "URGENT: High-Value Store Credit Authorization (${{AMOUNT}})",
          senderName: "Golf Town Regional Operations",
          senderEmail: "alerts@golftown.com",
          placeholders: ["{{CUSTOMER_NAME}}", "{{AMOUNT}}", "{{STORE_ID}}", "{{AUTHORIZER}}"],
          content: "URGENT SYSTEM ESCALATION: A high-value store credit transaction of {{AMOUNT}} for {{CUSTOMER_NAME}} (Store #{{STORE_ID}}) requires manager co-authorization."
        }
      ]
    };
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `golftown_global_backup_${new Date().toISOString().slice(0, 10)}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans pb-24 antialiased selection:bg-emerald-500 selection:text-white">
      ...
      <main className={`${viewMode === 'table' ? 'max-w-7xl' : 'max-w-3xl'} mx-auto px-4 mt-4 space-y-4 transition-all duration-300`}>
        {/* Printable Header Banner (Visible ONLY on print) */}
        <div className="hidden print:block print-header mb-6 pb-4 border-b-2 border-slate-900">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Golf Town Canada — Store Credit Audit Report</h1>
              <p className="text-xs text-slate-600 font-medium mt-1">
                Store Filter: #{selectedStoreId} • Printed On: {new Date().toLocaleString()}
              </p>
            </div>
            <div className="text-right">
              <p className="text-sm font-bold text-slate-900">Total Balance: ${stats.totalBalance.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
              <p className="text-xs text-slate-600 font-medium">Filtered Records: {filteredCustomers.length}</p>
            </div>
          </div>
        </div>
        {mobileTab === 'contacts' && (
